 ...
 * Vendored verbatim from deepseek-ai/deepseek-harness: packages/client/ui-primitives/src/index.ts
 * The full notice is web/deepseek/upstream/LICENSE.
 *
 * MODIFIED by the tetanus project: the two brand-art re-exports
 * (`FishLogo`, `BrandWordmark`) were removed, and `tools/vendor.py` refuses
 * to copy those two files. They draw DeepSeek's whale mark and the
 * `deepseek-official HARNESS` letterforms, which are trade marks. The MIT
 * licence grants copyright permission and says nothing about trade marks,
 * so shipping that art inside a differently-named product is the one thing
 * the licence does not cover. Nothing in the conversation view referenced
 * either symbol. */
/**
