...
| Outside the machine | An MCP client over stdio - handshake, discovery, invocation, a shutdown that leaves no child behind, and a bounded reconnect supervisor - with the servers a settings document declares started by the binary at boot, their tools offered beside the harness's own, a server that will not start named rather than silently absent, and none of them outliving the command that started them; a fetch with redirect, size and content-type policy above one transport seam; a search seam with one provider and a deterministic mock. A server that dies, hangs or answers nonsense fails that tool call with a class, and the turn carries on | Streamable HTTP for MCP, attachments for image results, more search providers, HTML converted to markdown |
...
| Config | Layered resolution with provenance (`default < file < env < flag`), reading a `settings.yaml` or `.json` document under the harness home - or the one `--settings <path>` names - re-reading it at run time, and resolving the engine's own settings out of it, which every subcommand that builds an engine boots from - the provider, model, step budget and journal root a turn runs on included - with the flags it was given over it on the `flag` layer; per-namespace schemas that refuse a scalar where a section belongs and decide which values are credentials by declaration rather than by name; a watcher joined to that re-read, so an edit reaches a running harness once the file has stopped moving; and writing the document back atomically | Profiles, bundles, patch overlays, comment-preserving writes |
