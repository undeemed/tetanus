...
| Process execution | One command through a resolved shell backend (bash, or PowerShell where a host has it), argv without a shell re-split, an environment the caller listed, output bounded to its tail and streamable while it runs, and a timeout that kills the whole process group; persistent shells that keep `cd` and exported variables between tool calls and report a death rather than restarting under the caller; persistent *terminals* on a real pseudo-terminal, where a command settles when the shell announces its own prompt - with that command's exit status - a `^C` reaches the command rather than the shell, and a bounded scrollback pages back from the newest line | Background jobs, spill files, a screen model for programs that draw with cursor movement |
