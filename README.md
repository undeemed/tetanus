...
Every printable key belongs to that row, `?` and `q` included, so the commands are still the chat's own: `/help`, `/exit`, `//text` to ask a question that opens with a slash, and `/find word` to look back through what was said - ctrl-n and ctrl-p walk the matches, Escape ends the search.
