...
| Code runtime | A model-written program evaluated once, on a worker thread, under fuel, a wall-clock ceiling and one output ledger; a runaway loop is stopped and its thread reclaimed. A remote backend behind the same trait submits, polls, fetches and cancels. Registered as `run_code`, so a program that fails is a failed tool call and the turn survives | A real-language backend, which needs the sandbox modes first; typed rejection classes; async bindings |
