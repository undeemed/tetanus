...
Every printable key belongs to that row, `?` and `q` included, so the commands are still the chat's own: `/help`, `/exit`, `//text` to ask a question that opens with a slash, `/find word` to look back through what was said - ctrl-n and ctrl-p walk the matches, Escape ends the search - and `/keys`, which names every key the screen answers, the editing ones included.
Tab and Shift-Tab walk the turns themselves, a turn's opening line to the top of the screen, which is how you get back to something said thirty turns ago without scrolling through the twenty-nine in between.
