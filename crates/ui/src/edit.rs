...
impl Line {
    ...
    pub fn key(&mut self, key: Key) -> Typed {
        match key {
            ...
            Key::Backspace | Key::Ctrl('h') => self.erase(self.back(1), self.cursor),
            Key::Delete => self.erase(self.cursor, self.forward(1)),
            Key::Left | Key::Ctrl('b') => self.move_to(self.back(1)),
            Key::Right | Key::Ctrl('f') => self.move_to(self.forward(1)),
            Key::Home | Key::Ctrl('a') => self.move_to(0),
            Key::End | Key::Ctrl('e') => self.move_to(self.text.len()),
            Key::Alt('b') => self.move_to(self.word_back()),
            Key::Alt('f') => self.move_to(self.word_forward()),
            Key::Ctrl('u') => self.erase(0, self.cursor),
            Key::Ctrl('k') => self.erase(self.cursor, self.text.len()),
            Key::Ctrl('w') => self.erase(self.word_back(), self.cursor),
            ...
            Key::Ctrl('d') => self.erase(self.cursor, self.forward(1)),
            _ => Typed::Ignored,
        }
    }
