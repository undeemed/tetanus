...
impl tetanus_protocol::methods::Engine for Silent {
    ...
    // `session.fork` lost its default body when the engine began serving it,
    // so every implementor must name it now. Section 7.4 of the contract calls
    // this compile error the point of putting the surface on a trait.
    async fn session_fork(
        &self,
        _: tetanus_protocol::methods::SessionForkParams,
    ) -> Result<tetanus_protocol::types::SessionInfo, tetanus_protocol::rpc::RpcError> {
        unreachable!()
    }
