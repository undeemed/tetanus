...
| --- | --- |
| ~~A PTY~~ - **served**, `crates/exec/src/pty.rs`, TC-PORT-PTY-1..8: allocation with a controlling terminal, size and resize, signal delivery to the foreground process group, and a read loop that drains continuously so a fast writer loses nothing. The interactive behaviour *over* it - viewport, scrollback paging, an owner-scoped registry - is the next slice | The layer was the blocker; the tools on top are ordinary work now that it exists. The pipe-backed persistent shells stay, and stay the default: they are exact where prompt detection is a guess, and most tool calls are not interactive |
