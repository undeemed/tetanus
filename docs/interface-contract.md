...
#### 4.3.5 `delivered` means delivered, not obeyed

`terminal_signal` answers, and a surface draws, a line of the form `delivered SIGINT to foreground process group 4213`.
Until recently that sentence could be false in the way that matters: a harness started in the background handed every child an ignored `SIGINT`, `killpg` reported success, and nothing happened.
That is fixed - a child now starts with a signal disposition of its own, the ignorable signals reset and the mask emptied between `fork` and `exec` - and what remains is a narrower promise that a surface must not round up.

**A program is entitled to handle or ignore a signal.** `trap '' INT` is a legitimate thing for a script to do, and no harness can promise compliance.
What this one promises is that the disposition being exercised is the *program's own* rather than one it inherited from whoever launched the harness.
So a surface renders `delivered` as "the signal reached the command's process group" and never as "the command stopped"; whether it stopped is what the next read of the terminal shows.
...
#### 4.3.6 Where a backgrounded tool's output lives, provisionally

**Provisional.** This clause is written by the process lane against a store another row owns, and the `workflow/*`, `schedule/*`, `jobs/*` row may ratify it or replace it when `tool-jobs` lands.
It is written down rather than left in code because the alternative is a convention a later lane discovers in a call site, and a convention nobody chose is worse than one somebody may overrule.
If that row replaces it, the process lane's tools change with it and this section is rewritten in the version that does so; until then this is what a reader of a backgrounded job may rely on.

A call that starts work and returns before the work is finished has nowhere to put the output.
A tool result is written when the call returns and the bytes arrive after it, so a backgrounded command's output cannot be the result, and the durable place a session already has for output too large to carry is the spill artifact filed beside its own journal (`tetanus_core::spill`, one directory per session).
A backgrounded one-shot command therefore writes its output to such an artifact, opened when the command starts and holding the whole stream, and the tool result carries the job id rather than the output.

**The store keeps a final output and refuses a stream, and that is what splits the two.**
`tetanus_core::jobs::Job` has an `output` field, documented as "the producer's final output", and its module says plainly that it "stores the terminal output a producer hands over and not a stream".
So the split is the store's own: the *rendered result* - the bounded text a foreground call would have answered with, markers and all - goes in `output`, exactly as that field intends, and the unbounded stream goes to the artifact, because that is the thing the store declines to hold.

**The artifact's path lives on the job's `detail`, by convention and not by type.**
No field on `Job` is a path, `detail` is a free `Option<String>` documented as "whatever the producer said about how it ended", and nothing enforces this.
A job queued by anything else carries something else there or nothing at all, so a reader that expects an artifact must tolerate its absence rather than fault on it.

**A live job's artifact is known to the process running it, and a terminal job's is on the record.**
`queue` and `start` take no detail: the store has no way to record anything about a job between starting it and finishing it, so until `finish` writes one there is nowhere on the record to put the path.
The process that started the command therefore answers for its own live jobs, and the record answers for every job that has ended.
The consequence is worth stating rather than discovering: a job whose process died while it was running is closed as `Interrupted` by the store's own reopen, with no path on it, and its artifact is left on disk under the session's spill directory named for the job - recoverable by an operator, not by a reader following this convention.
Ratifying this convention would be a good moment to give the store a way to record a fact about a running job; replacing it makes the question moot.

`detail` carries a JSON object, and a reader ignores keys it does not know, for §5's reason.
One key is fixed here: `artifact`, the path of the spill artifact holding the command's combined output.
A JSON object rather than a bare path is what lets this grow - an exit code, a truncation notice, a second stream - without a type change in a row that did not ask for one.

**Collection is served from the process row's own tools until `tool-jobs` exists.**
A producer with no consumer is a capability nobody can reach, which this workspace has now shipped four times, so the tool that starts the work also answers for it.
When `tool-jobs` lands it reads the same records, and the two surfaces agreeing is the point of writing the convention down instead of embedding it in one crate's call site.
...
#### 4.7.3 Every surface answers `catalog.tools` from the same assembly

The presentation lane found three answers to "what can this build run" in one tree with no settings document: `tetanus tools` printed 26, the WebSocket carrier answered 1, and the stdio carrier answered 0.
The cause was composition rather than protocol - a served engine had been built from a registry composed without the workspace and session context the `fs`, `features` and `exec` sources need, and a source given none of those registers nothing, which leaves exactly `echo`.

So the rule is written down: **a surface that serves this contract builds its engine from the shipped assembly, and `catalog.tools` is that assembly's answer.**
`crates/cli/src/tools.rs` states the invariant it exists for - the registry is built in one place so `tetanus tools` cannot list a tool a run does not have - and a carrier that composes its own registry breaks it silently, because a client cannot tell an empty toolbox from an empty answer.
A case that asserts the carrier's answer against the binary's own listing is what keeps the two honest; there is no client-side substitute.

#### 4.7.2 A carrier seam another lane added, recorded rather than discovered

§4.7's ownership table gives `crates/rpc` to the engine lane and says neither lane edits the other's files.
The ACP lane edited one: `crates/rpc` gained `FrameSink` and `FrameHandler`, two traits that let a carrier hand raw frames to something other than the codec, which is what an ACP bridge riding the same connection needs.
It is additive, the carrier conformance suite is unchanged and green, and no wire type, method or code moves.

It is written here because the rule is a rule and the alternative is a reviewer finding it in a diff.
The general form, for the next lane that meets it: a change to another lane's file that adds a seam and removes nothing is disclosed in this document rather than negotiated privately, and a change that alters what an existing seam *does* is not that lane's to make at all.
...
| 1.0 | States, provisionally, where a backgrounded tool's output lives (§4.3.6): the rendered result in the job record's own `output` field, which is what that field is for, and the unbounded stream in the session's spill artifact, with the artifact's path on the record's `detail` as a JSON object whose `artifact` key is fixed here. A live job's path is answered by the process running it, because the store takes no detail between `start` and `finish`. No boundary change, no type changes and no version bump - both fields exist and nothing on the wire moves. It is provisional because the process lane is writing a convention over the `workflow/*`, `schedule/*`, `jobs/*` row's store: that row's `tool-jobs` does not exist, so the job store landed with no consumer, and a producer with no consumer is the defect this workspace has shipped four times. The alternative to stating it was leaving the convention in one crate's call site for the jobs lane to discover later, which is why it is here and marked rather than assumed; that row may ratify or replace it, and a replacement rewrites this section and the tools together. The store keeps a producer's *final* output and explicitly not a stream, which is the split this clause follows; the path is on `detail` by convention rather than in a field because adding a field to `Job` is a change to a type another row owns. The cases land with the implementation, in the process lane's own suite. |
...
| 1.0 | No boundary change. Records in §4.7.3 the rule behind a defect the presentation lane found and the engine lane fixed: one tree with no document answered "what can this build run" three different ways - 26 from `tetanus tools`, 1 over the WebSocket carrier, 0 over stdio - because a served engine had been composed from a registry built without the workspace and session context the `fs`, `features` and `exec` sources need, and a source given none of those registers nothing. The rule is that every surface serving this contract builds its engine from the shipped assembly, so `catalog.tools` and the binary's own listing are one answer; a client cannot tell an empty toolbox from an empty answer, which is why this needs a case rather than a convention. |
| 1.0 | No boundary change. Records in §4.7.2 that `crates/rpc` gained `FrameSink` and `FrameHandler` from the ACP lane - additive traits that let a carrier hand raw frames to something other than the codec, which a bridge riding the same connection needs. §4.7's ownership table gives that crate to the engine lane, so the edit is disclosed here rather than found in a diff, and the general form is stated with it: a change to another lane's file that adds a seam and removes nothing is disclosed, and a change that alters what an existing seam does is not that lane's to make. |
| 1.0 | Says in §4.3.5 what `delivered` means on a terminal signal, which a surface had been free to round up. A program may handle or ignore a signal - `trap '' INT` is legitimate - so no harness can promise compliance; what this one promises, since children stopped inheriting the harness's dispositions, is that the disposition being exercised is the program's own. A surface therefore renders `delivered` as "the signal reached the command's process group" and never as "the command stopped". No type changes. |
| 1.0 | Records a defect in `methods::ALL` (§4.2) rather than fixing it here, because the fix is a `crates/protocol` change and that lands with its own PR: `method::AGENT_STEER` is declared, is routed by the codec, has a default `Engine` body answering `NotImplemented` and has a capability string, and is **not in `ALL`**. That is the single mistake the constant's own doc comment says it cannot catch, and it is the one that matters most, because every completeness check written against `ALL` - including the SDK gateway's descriptor catalogue, whose TC-PORT-API-1 iterates it - silently skips the call. `APPROVAL_SET`, the other reserved call, is in the list, so this is an oversight rather than a policy about reserved calls. Whoever adds it should expect the cases that iterate `ALL` to start covering one more method, which is the point. |
