...
| --- | ---: | --- | --- | --- |
| `core/*` (agent-loop, session, tools, agent, system-prompt, scope) | 58 | Turn engine, session log, registry, four dispatch modes, prompt assembly as a waterfall, `{{name}}` interpolation, a configured tool order with a rest entry for what it does not name, resume from a cold journal, interrupt at the step boundary, a failing tool contained as a result the model reads, a turn closed on the journal whichever way it ended | Fork, scoped stores, tool permissions, a queued inbox with steering and latched wakes, cancellation inside a step, a `max-tokens` stop reason, header metadata (cwd, parent session, subagent origin, delegation depth), fiber disposal, containment of a throwing decision listener (an observer is contained; `serial` and `waterfall` stay loud by design), the prompt-variable registry and an assembly that carries it, runtime-context providers, scoped prompt layers, a settings key that configures the tool order, property tests for the inbox and the tool pipeline | ② (port list in section 4) |
