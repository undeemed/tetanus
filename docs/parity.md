...
| `llm/*` | 34 | DeepSeek adapter, streaming seam, token-free mock, a bounded retry policy with jitter, heuristic token pricing and the priced surface | Further providers, the executor that runs the retry policy against a live route, resolving a policy out of settings, a measurement anchored on real provider usage, the three token projections | ② |
