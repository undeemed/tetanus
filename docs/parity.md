...
| `llm/*` | 34 | DeepSeek adapter, streaming seam, token-free mock, a bounded retry policy with jitter resolved out of the settings document, the executor that runs it against a live route, heuristic token pricing and the priced surface, a stream that ends without `[DONE]` refused | Further providers, a policy set per provider rather than once for the engine, a measurement anchored on real provider usage, the three token projections, the empty-response classification (it needs an `LlmError` variant, and the presentation lane matches that enum exhaustively, so it is a contract change first) | ② |
